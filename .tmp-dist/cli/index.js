...
import { HarnessMemory, TaskContinuityTracker, buildMemoryContext, buildPlanningRequest, buildPolicyHints, buildSelfCritiquePrompt, injectHarnessContext, isHeavyTask, shouldSelfCritique, validateToolCall } from './core/intelligence.js';
...
function getToolSignature(name, args) {
    return `${name}:${JSON.stringify(args ?? {})}`;
}
function popLastUserIfPending(messages) {
    const last = messages[messages.length - 1];
    if (last?.role === 'user')
        messages.pop();
}
function isStableCachedResult(toolName, result) {
...
function isPerTurnDedupableTool(toolName) {
    return toolName === 'write_file' || toolName === 'edit_file';
}
function shouldPrintNotice(noticeSet, key) {
    if (noticeSet.has(key))
        return false;
    noticeSet.add(key);
    return true;
}
function getNoopToolResult(call) {
...
async function runSelfCritiqueIfNeeded(provider, messages, response, taskInput) {
    if (!shouldSelfCritique(response, taskInput))
        return response;
    const critiquePrompt = buildSelfCritiquePrompt(taskInput, response.content || '');
    try {
        const improved = await provider.sendMessage([...messages, { role: 'assistant', content: response.content || '' }, { role: 'user', content: critiquePrompt }], tools);
...
async function runPlanningPass(provider, messages, taskInput) {
...
export async function startChat(providerName, modelName) {
    const config = loadConfig();
    let currentProvider = providerName || config.DEFAULT_PROVIDER;
    const configKey = `${currentProvider.toUpperCase()}_MODEL`;
    let currentModel = modelName || config[configKey];
    let planningMode = 'auto';
    ...
    let messages = [{ role: 'system', content: fullSystemPrompt }];
    ...
    while (true) {
        ...
        if (trimmedInput.startsWith('/')) {
            ...
            if (command === '/models') {
                const selected = await safePrompt(() => search({
                    message: 'Select Model (Inspect or type to search)',
                    source: async (term) => {
                        if (!term)
                            return AVAILABLE_MODELS;
                        const t = term.toLowerCase();
                        return AVAILABLE_MODELS.filter(m => !(m instanceof Separator) && (m.value.model.toLowerCase().includes(t) || m.value.provider.toLowerCase().includes(t)));
                    }
                }));
                if (!selected)
                    continue;
                ...
                config[providerModelKey] = currentModel;
                config.DEFAULT_PROVIDER = currentProvider;
                saveConfig({ [providerModelKey]: currentModel, DEFAULT_PROVIDER: currentProvider });
                renderWelcome(currentProvider, currentModel);
        ...
        executionInput = injectHarnessContext(executionInput, memoryContext, policyHints);
        messages.push({ role: 'user', content: executionInput });
        const turnToolResultCache = new Map();
        const turnNoticeCache = new Set();
        ...
        try {
            while (true) {
                ...
                if (provider.streamMessage) {
                    ...
                    let tokenCount = 0;
                    try {
                        const result = await interrupt.run(() => provider.streamMessage(messages, tools, (_chunk) => {
                            tokenCount++;
                            if (tokenCount === 1) {
                                stop();
                        ...
                        stop();
                        if (result.cancelled) {
                            if (interrupt.isHardCancelled() && !producedOutputThisTurn)
                ...
                const msgObj = { role: 'assistant', content: response.content || '' };
...
export async function runOnce(prompt, providerName, modelName) {
    const config = loadConfig();
    const currentProvider = providerName || config.DEFAULT_PROVIDER;
    const configKey = `${currentProvider.toUpperCase()}_MODEL`;
    const currentModel = modelName || config[configKey];
    const memory = new HarnessMemory();
    const continuity = new TaskContinuityTracker();
    ...
    const baseMessages = [{ role: 'system', content: systemContent }];
    ...
    const messages = [...baseMessages, { role: 'user', content: executionInput }];
    const toolResultCache = new Map();
    const interrupt = createTurnInterruptController();
