...
export function composeSystemPrompt(projectContext, contextHeader) {
    return projectContext
        ? `${SYSTEM_PROMPT}\n\n${contextHeader}\n${projectContext}`
        : SYSTEM_PROMPT;
}
export async function injectMentionedContextWithMetadata(input) {
    ...
    if (injections.length === 0)
        return { content: input, loadedFiles };
    return { content: input + '\n\n' + injections.join('\n\n'), loadedFiles };
}
export async function injectMentionedContext(input) {
