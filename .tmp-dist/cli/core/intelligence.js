...
function sanitizeMentionToken(token) {
    return token
        .replace(ANSI_SGR_RE, '')
...
function extractMentionedFiles(input) {
    const found = [];
    let atMatch;
...
export class HarnessMemory {
    ...
    retrieve(query, limit = 3) {
        ...
        const scored = this.items.map(item => {
            let score = 0;
            for (const k of qk)
                if (item.keywords.includes(k))
                    score += 2;
            if (qk.some(k => item.source.toLowerCase().includes(k)))
                score += 3;
...
export class TaskContinuityTracker {
    ...
    buildContextBlock() {
        if (!this.activeObjective && this.focusedFiles.length === 0)
            return '';
    ...
    validateToolCall(call) {
        if (call.name !== 'write_file' && call.name !== 'edit_file' && call.name !== 'read_file')
            return null;
    ...
    isFollowup(text) {
        ...
        if (short && FOLLOWUP_RE.test(text))
            return true;
        if (short && /^\s*(and|also|plus)\b/i.test(text))
            return true;
        return false;
    }
    addFocusedFile(filePath) {
...
export function isHeavyTask(input) {
    const text = input.trim();
    if (!text)
        return false;
    if (QUESTION_RE.test(text) && text.length < 120)
        return false;
...
export function buildPlanningRequest(userInput) {
    return [
...
export function buildPolicyHints(userInput) {
    const hints = [];
...
export function buildMemoryContext(userInput, memory) {
    const hits = memory.retrieve(userInput, 3);
    if (hits.length === 0)
        return '';
    return hits.map((h, i) => `- [${i + 1}] ${h.kind}:${h.source}\n${h.content}`).join('\n\n');
}
export function injectHarnessContext(userInput, memoryContext, policyHints) {
    if (!memoryContext && policyHints.length === 0)
        return userInput;
...
export function shouldSelfCritique(response, userInput) {
    if (!response?.content || response.toolCalls?.length)
        return false;
    if (response.content.length < 50)
        return true;
    if (LOW_CONFIDENCE_RE.test(response.content))
        return true;
    return /\b(implement|fix|build|refactor|create|update|redesign)\b/i.test(userInput);
}
export function buildSelfCritiquePrompt(userInput, previousAnswer) {
    return [
...
export function validateToolCall(call, toolDefs) {
