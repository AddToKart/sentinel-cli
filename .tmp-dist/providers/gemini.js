...
export class GeminiProvider {
    ...
    async sendMessage(messages, tools = []) {
        ...
        if (calls && calls.length > 0) {
            return {
                ...
                toolCalls: calls.map((c) => ({ name: c.name, args: c.args, id: c.name })),
            };
        }
    ...
    async streamMessage(messages, tools, onChunk) {
        ...
        if (calls && calls.length > 0) {
            return { content: fullText, toolCalls: calls.map((c) => ({ name: c.name, args: c.args, id: c.name })) };
        }
